...
static v4sf pow_ps(const v4sf & x, const v4sf & y)
{
	v4sf tmp = log_ps(x);
	return exp_ps(vmulq_f32(y, tmp));
}
...
static void sincos_ps(v4sf x, v4sf *ysin, v4sf *ycos)
{
...
{
	v4sf ysin, ycos;
	sincos_ps<SinCos::Sin>(x, &ysin, &ycos);
	return ysin;
}
...
{
	v4sf ysin, ycos;
	sincos_ps<SinCos::Cos>(x, &ysin, &ycos);
	return ycos;
}

#ifdef vdivq_f32
static v4sf tan_ps(v4sf x)
{
	v4sf s, c;
	sincos_ps<SinCos::Sin | SinCos::Cos>(x, &s, &c);
