...
template<int Type = SinCos::Sin | SinCos::Cos>
...
{
	__m256 s, c;
    _my_mm256_sincos_ps<SinCos::Sin>(x, &s, &c);
	return s;
}
...
{
	__m256 s, c;
	_my_mm256_sincos_ps<SinCos::Cos>(x, &s, &c);
	return c;
}
...
{
    __m256 s, c;
    _my_mm256_sincos_ps<SinCos::Sin | SinCos::Cos>(x, &s, &c);
...
{
    __m256 s, c;
    _my_mm256_asincos_ps<SinCos::Sin>(x, &s, &c);
    return s;
}
...
{
    __m256 s, c;
    _my_mm256_asincos_ps<SinCos::Cos>(x, &s, &c);
    return c;
}
...
static __m256 _my_mm256_atan_ps(__m256 x)
{
    ...
    __m256 y = _my_mm256_select(big, _mm256_set1_ps(PIO2F),
    ...
    return _my_mm256_select(signBit, _my_mm256_swap_sign(y), y);
}
